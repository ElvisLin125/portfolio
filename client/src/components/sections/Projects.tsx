      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col gap-32 md:gap-48">
          {projects.map((project, index) => (
            ...
            >
              {/* Image Container */}
              <div className={`w-full md:w-3/5 overflow-hidden relative aspect-[4/3] md:aspect-auto md:h-[80vh] ${index % 2 !== 0 ? 'md:order-2' : ''}`}>
              ...
              {/* Text Content */}
              <div className={`w-full md:w-2/5 flex flex-col justify-center ${index % 2 !== 0 ? 'md:order-1 md:items-end md:text-right' : ''}`}>
                ...
                <div className={`flex flex-wrap gap-2 mt-8 max-w-md ${index % 2 !== 0 ? 'justify-end' : ''}`}>
                  {project.techStack?.map((tech) => (
                    <span key={tech} className="font-sans text-[10px] tracking-wider uppercase bg-white/5 px-3 py-1 text-white/60">
                      {tech}
                    </span>
                  ))}
                </div>
